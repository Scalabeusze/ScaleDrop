...
package com.scaledrop.sdiam.configuration
...
class SecurityConfigurationTest extends WiremockTestBase {
  ...
  // ACCOUNTS
  def 'should not allow access to accounts endpoint without credentials'() {
    expect:
    mockMvc.perform(get(ACCOUNTS_ENDPOINT))
        .andExpect(status().isUnauthorized())
  }

  def 'should not allow access to accounts endpoint with invalid credentials'() {
    expect:
    mockMvc.perform(get(ACCOUNTS_ENDPOINT)
        .with(httpBasic(WRONG_USER, WRONG_PASSWORD)))
        .andExpect(status().isUnauthorized())
  ...
  def 'should not allow access to accounts endpoint with documentation credentials'() {
    expect:
    mockMvc.perform(get(ACCOUNTS_ENDPOINT)
        .with(httpBasic(DOCUMENTATION_USERNAME, DOCUMENTATION_PASSWORD)))
